...
export default function UpstoxAuthToken({ clientId, redirectUri , access_token}: Props) {
  const [error, setError] = useState<string | null>(null);
  const [showRelogin, setShowRelogin] = useState(false);
const [loading, setLoading] = useState(true);
  useEffect(() => {
    try {
      if (!clientId || !redirectUri || !access_token) {
        setError("Configuration error. Please try later.");
            setLoading(false);
        return;
      }
       const accessTokenAuthCookie =access_token;
       // cookies().get("Authorization")?.value ?? "";
      const bearer = "Bearer ";
               let indxBearer =  accessTokenAuthCookie.indexOf(bearer)
      let receivedT = accessTokenAuthCookie.substring(indxBearer);
      let isValid = false;
        if(receivedT){
             let code_split =    receivedT.split(".");   // isValidJwtStructure(auth_code)
              code_split.length ==3 ;
              isValid = true;
        }
       let data = {
      ...
      const attempted = localStorage.getItem("upstox-auth-attempted");
     ...
     // ✅ Already authenticated → go to fallback
      ...
      // 🔴 Attempted before but no token → stop loop attempted && 
      if (!token) {
    ...
    /*  // 🟢 First-time OAuth initiation
      localStorage.setItem("upstox-auth-attempted", "true");
      ...
      console.error("Upstox auth error:", e);
  ...
  const handleRelogin = () => {
    try {
      localStorage.removeItem("upstox-auth-attempted");
      //localStorage.removeItem("fyersToken");
      //     window.location.replace("/upstox-fallback");
         const params = new URLSearchParams({
        client_id: clientId,
        redirect_uri: redirectUri,
        response_type: "code",
  ...
  return (
    <>
      <Head>
        <title>Upstox Token Assembly</title>
           
       
           {/*   Upstox  UI   AND   JS    IMPORTANT   SCRIPT  LINK */}
     ...
     <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-black px-4">
        ...
        <h1>Upstox Token Assembly </h1>
          <p className="text-slate-400 mt-2 text-sm">
            Connecting securely to your Upstox account
          </p>
        {/* 🔴 Error */}
          {error && (
            <motion.div
          ...
          {loading && !error && !showRelogin && (
            ...
            >
              ...
              <p className="text-slate-400 text-sm flex items-center gap-2">
                ...
                Redirecting to Upstox
                 <motion.button
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                       onClick={() => window.close()}
                       className="mt-6 w-full flex items-center justify-center gap-2 bg-brandgreen-300 hover:bg-brandgreen-400 font-bold  py-3 rounded-xl text-brandgreen"
                    >
                           <strong>  OK</strong>
...
if (!query.access_token) {
    return {
      redirect: {
        destination: "/upstox-error?reason=missing_auth_code",
        permanent: false,
      },
    };
  }
