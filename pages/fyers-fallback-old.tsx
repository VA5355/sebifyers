...
export default function FyersCallback(props: Props) {
  ...
  const handleProceedAccess = () => {
    window.location.assign(
      `http://192.168.1.3:8888/.netlify/functions/netlifystockfyersbridge/api/fyersgetaccessauthcode?auth_code=${auth_code}`
    );
  };
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  ...
  return {
    props: {
         clientId: process.env.FYERS_CLIENT_ID ?? "",  //  JDK56F3KP5-200 7GSQW68AZ4-100 
        lastlogin: new Date ,
      ...
      secret_key: process.env.FYERS_SECRET_KEY ?? "",  //"G75gMipThUCNWpLA"; 		 // "MGY8LRIY0M"; 
    },
  };
};
