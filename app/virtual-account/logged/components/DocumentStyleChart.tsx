...
FYERS_SECRET_KEY=V72MPISUJC
#FYERS_SECRET_KEY=4LXEKKMFUL
FYERS_REDIRECT_URI=https://192.168.1.7:8888/.netlify/functions/netlifystockfyersbridge/api/fyerscallback?task=generate
...
"latestPrice":0,"open":0,"high":0,"low":0,"close":0,"week52High":0,"week52Low":0},"price":0},"ttl":1782097338298}
 ...
 async function loadFyersWebSdkSearchSymbol() {
    ...
    try {  
        let symbol = undefined;
         ...
         const cachedQuote = StorageUtils._retrieve(CommonConstants.LASTSTOCKQUOTENSEYAHOO);
              ...
              {
                symbol = cachedQuote.data["symbol"] ;
                console.log(" Symbol   found in  LAST STOCKQUOTE NSEYAHOO : "+symbol);
              }

         }catch(ert)    {
             console.log(" Symbol not found in  LAST STOCKQUOTE NSEYAHOO :");
         }
        if(symbol !== undefined) {   
            ...
            if(acess_token !==undefined && acess_token !==null && acess_token !==''){
                ...
                fyers.getHistory(inp).then((response :any )=>{
                    ...
                    const data: CandlestickData<Time>[] = [];
                        for (let i = 0; i < openHLC.length; i++) {
                            const [time, open, high, low, close] = openHLC[i] ;
                 ...
                 console.log('Seems NO CACHED symbol to fetch  from fyers-web-sdk-v3 ' )
                 console.log(' FETCHING DUMMY FALLBACK STOCK DATA ' )
                 return generateFallbackData();
        }
     }catch(eree:any )   {
          console.log('Seems could not initialize fyersModel from fyers-web-sdk-v3 '+ JSON.stringify(eree))
            return generateFallbackData();
    }      
 }
  ...
  async function loadSPYData(): Promise<CandlestickData<Time>[]> {
    console.log(" load SPY Data ......... ")
    try {

      let fyersData = await loadFyersWebSdkSearchSymbol ();
     if(fyersData === undefined || !Array.isArray(fyersData)) { 
      ...
      return data;
          ...
          console.log(" available Fyers  Data ......... ")
        return fyersData;
     }
    } catch (error) {
  ...
  class DrawingDemo {
    ...
    private currentTool: string | null = "select" ; //null;
    ...
    private setupToolbar(): void {
      ...
      document.getElementById('clear-all')?.addEventListener('click', () => {
       ...
       document.getElementById('reset-theme-chart')?.addEventListener('click', () => {
         this.chart.applyOptions({ layout: {
          background: { color:  '#ffffff' },
          textColor:'#050558',
        }});
        let selected =  document.querySelector(".drawing-item .selected ");
        if(selected){
            let dItem  = selected as HTMLElement
            dItem.style.backgroundColor = "#2962ff";
        }
        
        });
      document.getElementById('enable-chart')?.addEventListener('click', () => {
        let chartBtn  = document.getElementById('enable-chart');
        let isEnabled = chartBtn?.classList;
        let isChartDisabled = false; 
        if (isEnabled?.contains('danger')) {
        ...
        if(isChartDisabled){
            this.enableChartMovement();
        chartBtn?.classList.add('primary');
            chartBtn?.classList.remove('danger');
            if(chartBtn !==null) {
                  chartBtn.textContent = "Enabled";
            }
          
        }
        else {
            this.enableDrawingMode();
         chartBtn?.classList.add('danger');
            chartBtn?.classList.remove('primary');
            if(chartBtn !==null) {
                chartBtn.textContent = "Disabled";
          }
        }
    ...
    private enableChartMovement   ()  {
        //Enable chart horizontal movement after drawing is completed
        //When the drawing finishes, switch back to navigation:
        console.log("enabling enableChartMovement   ....Enable chart horizontal movement")
        this.chart?.applyOptions({
                handleScroll: {
                  mouseWheel: true,
                  pressedMouseMove: true,
   ...
   private enableDrawingMode  ()  {
      // When a tool is selected, disable chart movement
      console.log("enabling enableDrawingMode   ....disable chart movement ")
        this.chart?.applyOptions({
                handleScroll: {
  ...
  // check toolType after some timeout as on first click the currenTool is null
    ...
    private selectTool(toolType: string): void {
      ...
      if(this.currentTool ==="select") {
        ...
        console.log("selectTool     this.currentTool  "+this.currentTool)
        this. enableDrawingMode();
     }
      ...
      if (this.currentTool === toolType) {
       ...
       // this.chart.applyOptions({ crosshair:{    mode:1  },  handleScroll:{  pressedMouseMove:false  }  });
    ...
    private setupChartInteraction(container: HTMLElement): void {
      ...
      container.addEventListener('mousemove', (event) => {
        ...
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top; 
        console.log("container.addEventListener   ....mousemove")
        console.log("    getBoundingClientRect   ....mousemove  x "+ x + " y "+y)
        console.log("   this.currentTool  ....mousemove   "+this.currentTool )
        //applyOptions({ crosshair:{    mode:1  },  handleScroll:{  pressedMouseMove:false  }  });
        const time = this.chart.timeScale().coordinateToTime(x);
        const price = this.series.coordinateToPrice(y);
        if(this.currentTool ==="select") {
            this.enableChartMovement();
        ...
        this.updatePreviewWithMouse({ time, price });
            ...
            // Handle mouse up for rubber-band preview
            container.addEventListener('mouseup', (event) => {
                if (!this.currentTool || this.pendingAnchors.length === 0) return;
                console.log("container.addEventListener   ....mouseup")
                const rect = container.getBoundingClientRect();
                const x = event.clientX - rect.left;
                const y = event.clientY - rect.top; 
                console.log("    getBoundingClientRect   ....mouseup  x "+ x + " y "+y)
                console.log("   this.currentTool  ....mouseup   "+this.currentTool )
                if(this.currentTool ==="select") {
                    this.enableChartMovement();
                    }
                 else {
                  ...
                  // Enable Disable tthe Header Button 
         document.getElementById("enable-chart")?.click();
                /*
                      handleScroll: {
            mouseWheel:true,
            pressedMouseMove:false,
            ...
            vertTouchDrag:false
                ...
                */
                this.chart.applyOptions({ crosshair:{    mode:1  },  handleScroll:{  pressedMouseMove:true  }  });
                const time = this.chart.timeScale().coordinateToTime(x);
                const price = this.series.coordinateToPrice(y);
       ...
       container.addEventListener('mousedown', (event) => {
        ...
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top; 
        console.log("container.addEventListener   ....mousedown")
        console.log("    getBoundingClientRect   ....mousedown  x "+ x + " y "+y)
        console.log("   this.currentTool  ....mousedown   "+this.currentTool )
        if(this.currentTool ==="select") {
            this.enableChartMovement();
            }
         else {
    ...
    private updateDrawingList(): void {
      ...
      const drawings = this.drawingManager.getAllDrawings().filter((d:any) => d.id !== this.previewId);
    ...
    private getDrawingText(drawing: IDrawing): string {
      switch (drawing.type) {
        ...
        case 'table':
          return (drawing as Table).getRows().map((r:any) => r.join('\t')).join('\n');
        default:
          return '';
      }
    }
  ...
  async function init() {
    ...
    console.log(" Document Chart Element found   ")
    let  data = await loadSPYData();
    if(data !== undefined){
        console.log(" Chart SPY Data loaded from CSV   ")
    }
    else {
        // create candle stick series , use invisible chart 
          const container = document.getElementById('chartIn')!;
          ...
          const candleData = [
            { time: '2026-06-10', open: 150.5, high: 152.3, low: 149.8, close: 151.8 },
            { time: '2026-06-11', open: 151.8, high: 153.5, low: 151.2, close: 153.1 },
            { time: '2026-06-12', open: 153.1, high: 154.0, low: 150.5, close: 151.2 },
            { time: '2026-06-13', open: 151.2, high: 152.8, low: 149.1, close: 149.5 },
            { time: '2026-06-14', open: 149.5, high: 151.0, low: 148.0, close: 150.3 },
            { time: '2026-06-15', open: 150.3, high: 155.2, low: 150.1, close: 154.7 },
            { time: '2026-06-16', open: 154.7, high: 156.4, low: 153.8, close: 155.9 },
          ];
...
export const DocumentStyleChart = { draw : () => {   if (document !==undefined  ){
                              console.log(" Document Object defined drawing should start  ")
                                             document.addEventListener('DOMContentLoaded', init) 
                                             console.log(" Document Object manually triggered  ")
                                        ...
                                        console.log(" Document Object undefined ")
                                    }
 ...
 /*  document.dispatchEvent(new Event('DOMContentLoaded', {
                                            ...
                                            */
 /* export function  DocumentStyleChart  ()  {
