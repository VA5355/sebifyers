...
import DemoTradeModal from "./demotradermodal";
    ...
    async function fetchData() {
      ...
      const res =     await Promise.resolve( 
       ...
       // const json = await res.json();
        // @ts-ignore
        const chartDates = res[mapper["TIME_SERIES_INTRADAY"]]
          let formatted: StockData[] = Object.entries(chartDates)
        .map(([time, v]: any) => ({
          date: time,
          open: Number(v["1. open"]),
          high: Number(v["2. high"]),
        ...
        if (!formatted || formatted.length === 0) {
         // setError(IBM_SAMPLE);
            let formatted = await formatErrorSmall();
        //    formatted = { ...formatted , ... {  buyClick: setShowDemoModal(true) , sellClick: setShowDemoModal(true) }  }
          setData(formatted);
        } else {
         //     formatted = { ...formatted , ... {  buyClick: setShowDemoModal(true) , sellClick: setShowDemoModal(true) }  }
          setData(formatted);
