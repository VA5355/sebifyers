...
export default function SubscribeSwipeButton({ onSwipe }) {
    const isMobile = useIsMobile();

  return (
    <motion.div
      className="relative w-full max-w-[320px] sm:max-w-[380px] md:max-w-[420px] lg:max-w-[480px] mx-auto select-none px-2">
      {/* Motion streak */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.3 }}
      ...
      <motion.button
        drag="x"
        dragConstraints= { !isMobile ? { left: -220, right: 140 }: { left: -40, right: 40}}
        dragElastic={0.1}
        whileTap={{ scale: 0.96 }}
        onDragEnd={(e, info) => {
      ...
      >
        {/* Text */}
        <span className="whitespace-nowrap text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl">
         Subscribe your Swipe
        </span>
  {/* Right Icons */}
