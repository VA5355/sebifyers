...
import { useSocket } from "@/providers/FyersIndicesSoketProvider";
...
let  stringMap  = null;
 ...
 let fyersIndicesSocketPollCnt = 0;
 let fyersIndicesSocketPoll = 0;
/*
   useEffect(() => {
...
export const startFyersIndicesProviderSocketIO = (connectionStatus,inSocketObj, onFeed) => {
  let socket = inSocketObj;
 return async (dispatch) => {
     if(!socket) { 
        console.log("startFyersIndicesProviderSocketIO socket is not available as passed parameter inSocketObj ")
         // socket = useSocket();
     }
  if (!connectionStatus) {
            try { 
                  userLoggedIn();
                if (userAuthCode && userAuthCode !== null && userAuthCode !== undefined) {
                    // User is logged in TRY a COUPLE of TIME to establish the socket 
                    fyersIndicesSocketPoll  =  setInterval( async() => {
                          if(!socket) { 
                           console.log("startFyersIndicesProviderSocketIO polling for active  socket ")
                         // socket = useSocket();
                        }
                          if(socket !== undefined && socket !== null){
                            console.log("startFyersIndicesProviderSocketIO starts Fyers Indices feed");
                            ...
                            console.log("startFyersIndicesProviderSocketIO is awaiting  socket  availability ");
                         }
                        // ITERATE or TRY socket polling for 5 times 
                        if ( fyersIndicesSocketPollCnt < 5 ){ 
                             console.log("startFyersIndicesProviderSocketIO socket poll "+fyersIndicesSocketPollCnt );
                             fyersIndicesSocketPollCnt++;
                        }
                        else {    clearInterval(fyersIndicesSocketPoll);   }
              ...
              console.log("loginFeed.action : startFyersIndicesProviderSocketIO either user not logged or FyersIndicesProviderSocket connection issue   ")
            }
